...
async def on_message(message):
	...
	if result_bot_help is not None:
		...
		request = requests.get(url + urllib.parse.quote(result_message), headers=USER_AGENT)
		request_gallery = requests.get(url_gallery + urllib.parse.quote(result_message) + url_gallery_end, headers=USER_AGENT)
		try:
			...
			return
		result_search_url_gallery = requests.get(result_url_gallery, headers=USER_AGENT)
		...
		request_end_link = requests.get(result_table_href, headers=USER_AGENT)
